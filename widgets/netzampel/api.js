...
module.exports = {
  async getData({ homey }) {
    ...
    const pvPower      = cap(sun2000,    'measure_power',                  null) ?? cap(sun2000em,  'measure_power',  0);
    const gridPower    = cap(sun2000,    'measure_power.grid_active_power', null) ?? cap(pmEmma,     'measure_power',  0);
    const batteryPower = cap(luna2000,   'measure_power',                  null) ?? cap(luna2000em, 'measure_power',  0);
