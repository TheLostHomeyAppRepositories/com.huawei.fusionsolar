  ...
  async _solarTickInner() {
    ...
    if (engineCharging) {
      ...
      if (!step) {
        ...
        // the stop grace, so brief dips don't kill the session.
        this._pendingStepAmps = null;
        this._pendingStepSince = null;
      ...
      if (step.amps !== currentAmps) {
        // Step up/down only after the new level holds for step_hold —
        // otherwise passing clouds would thrash the ladder.
