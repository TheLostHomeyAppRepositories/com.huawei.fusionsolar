...
function _noteBatchSplit(start, words, count, err) {
  const reason = _errReason(err);
  ...
  _readErrLogged.set(key, Date.now());
  // eslint-disable-next-line no-console
  console.log(`[modbus] batch ${start}/${words} (${count} registers) failed: ${reason} — splitting`);
}
...
module.exports = {
  ...
  buildReadPlan,
  parseBuffer,
  _resetStaticCache: () => _staticCache.clear(),
};
