...
// Spans whose last attempt failed: start address → { count, at }. Only the falling edge
// used to be logged, so a burst of failures was followed by silence and there was no way
// to tell a fault that had passed from one still going on — the more so since the
// driver's own "Poll OK" line is throttled to a 15-minute heartbeat.
const _readFailedAt = new Map();
// Inside one poll a failed batch is immediately re-read as halves, and the first half
// shares the failed span's start address. Announcing a recovery 100 ms later would be
// describing the bisection, not a fault that passed, so only a gap that outlives the
// cascade counts as one.
const RECOVERY_MIN_MS = 5000;

function _noteSpanFailed(start) {
  const f = _readFailedAt.get(start) || { count: 0, at: Date.now() };
  f.count += 1;
  _readFailedAt.set(start, f);
}

// Counterpart to the two "failed" lines below: says the span answers again, and for how
// long it did not. Counting happens on every failure; only the logging is throttled, so
// the number here is the true one.
function _noteSpanOk(start, words, count) {
  const f = _readFailedAt.get(start);
  if (!f) return;
  _readFailedAt.delete(start);
  const ms = Date.now() - f.at;
  if (ms < RECOVERY_MIN_MS) return;
  // eslint-disable-next-line no-console
  console.log(`[modbus] batch ${start}/${words} (${count} registers) reading again after ${f.count} failed attempt(s) over ${Math.round(ms / 1000)}s`);
}

function _noteReadError(address, length, err) {
  _noteSpanFailed(address);
...
async function _readSpan(group, client, result, allowRetry) {
  ...
  try {
    ...
    _noteSpanOk(start, words, group.length);
  } catch (err) {
...
module.exports = {
  ...
  _resetStaticCache: () => { _staticCache.clear(); _readFailedAt.clear(); _readErrLogged.clear(); },
  // Test hook: pretend the recorded failures happened `ms` earlier, so the recovery
  // threshold can be crossed without the test sleeping through it.
  _ageFailuresForTest: (ms) => { for (const f of _readFailedAt.values()) f.at -= ms; },
};
