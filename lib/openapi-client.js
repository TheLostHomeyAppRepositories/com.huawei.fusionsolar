...
async function getDevRealKpi(baseUrl, token, devIds, devTypeId) {
  ...
  if (data.success && Array.isArray(data.data) && data.data.length > 0) {
    return { expired: false, devices: data.data };
  }

  if (isSessionExpired(data.failCode)) return { expired: true, devices: [] };
  ...
  if (!data2.success) return { expired: isSessionExpired(data2.failCode), devices: [] };

  const devices = Array.isArray(data2.data) ? data2.data : [];
  return { expired: false, devices };
}
...
async function controlChargeDischarge(baseUrl, token, plantCode, dispatchSwitch, opts = {}) {
  const task = { plantCode, dispatchSwitch };
  if (opts.controlType !== undefined) task.controlType = opts.controlType;
  if (opts.targetSOC !== undefined)   task.targetSOC   = opts.targetSOC;
  if (opts.dispatchTime !== undefined) task.dispatchTime = opts.dispatchTime;
  if (opts.powerDispatch !== undefined) task.powerDispatch = opts.powerDispatch;
  ...
  if (!data.success && data.failCode === 2) {
    const msg = data.data?.result?.[0]?.description ?? data.message ?? 'Task failed';
    throw new Error(msg);
  }
  if (isSessionExpired(data.failCode)) throw new Error('Session expired');
  return data;
}

async function controlBatteryMode(baseUrl, token, plantCode, operationMode, opts = {}) {
  const task = { plantCode, operationMode };
  if (opts.redundantPVEnergyPriority !== undefined) task.redundantPVEnergyPriority = opts.redundantPVEnergyPriority;
  if (opts.allowedAcChargePower !== undefined) task.allowedAcChargePower = opts.allowedAcChargePower;
  if (opts.chargingAndDischargingTimeWindow !== undefined) task.chargingAndDischargingTimeWindow = opts.chargingAndDischargingTimeWindow;

  const { data } = await post(
    baseUrl,
    '/rest/openapi/pvms/nbi/v1/control/battery/mode/async-task',
    { tasks: [task] },
    token,
  );

  if (!data.success && data.failCode === 2) {
    const msg = data.data?.result?.[0]?.message ?? data.message ?? 'Task failed';
    throw new Error(msg);
  }
  if (isSessionExpired(data.failCode)) throw new Error('Session expired');
  return data;
...
async function controlBatteryDispatch(baseUrl, token, plantCode, batteryDn, dispatchPower) {
  const { data } = await post(
    baseUrl,
    '/rest/openapi/pvms/nbi/v1/control/battery/battery-dispatch/async-task',
    { plantCode, batteryDn, dispatchPower },
    token,
  );

  if (!data.success) {
    const msg = data.message ?? 'Task failed';
    throw new Error(msg);
  }
  if (isSessionExpired(data.failCode)) throw new Error('Session expired');
  return data;
...
async function controlActivePower(baseUrl, token, plantCode, controlMode, controlInfo) {
  const task = { plantCode, controlMode: String(controlMode) };
  if (controlInfo) task.controlInfo = controlInfo;
...
async function controlBatteryConfig(baseUrl, token, plantCode, config) {
  const { data } = await post(
    baseUrl,
    '/rest/openapi/pvms/nbi/v1/control/battery/configuration/async-task',
    { tasks: [{ plantCode, batteryConfigurationInfo: config }] },
    token,
  );
