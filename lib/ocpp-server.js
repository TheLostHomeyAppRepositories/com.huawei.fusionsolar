...
class OcppServer {
  ...
  _onConnection(ws, req) {
    ...
    // heartbeat — and never sent a single MeterValues, so every current, voltage and
    // temperature stayed blank. Its log had no BootNotification at all. A charger sends
    // that when IT boots, and this one had been running for hours; the app had restarted,
    // so the charger merely reconnected the WebSocket. Configuration hung off the boot
    // message alone, so it never ran, and a charger that is never told what to sample
    // samples nothing.
    //
    // The two paths are both kept: a real boot still reconfigures after its own delay,
    // because a charger that has just started up may not accept settings immediately.
    // ChangeConfiguration is idempotent, so arriving twice costs nothing.
