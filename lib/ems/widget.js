...
module.exports = {
  ...
  // The widget edits the one threshold there is. stopSoc writes the ramp lower point,
  // which is also the hard stop (see _batteryZones); normalSoc is still accepted so an
  // older widget build keeps working and lands on the same setting.
  async setEmsBatteryZones({ stopSoc, normalSoc }) {
    const cfg = this._getConfig();
    const v = (stopSoc !== undefined && stopSoc !== null) ? stopSoc : normalSoc;
    if (v !== undefined && v !== null) cfg.share_soc_low = Math.max(0, Math.min(100, Number(v)));
