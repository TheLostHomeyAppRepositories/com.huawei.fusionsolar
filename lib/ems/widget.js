...
module.exports = {
  ...
  async getEmsControllableStatus(id) {
    ...
    if (kind === 'charger') {
      ...
      // Der Abschnitts-Schalter ueberstimmt den Geraete-Schalter: steht er aus, steuert das
      // EMS dieses Geraet nicht, egal was hier gesetzt ist (chargerControl.js kehrt vorher
      // zurueck). Die Kachel zeigt daher die tatsaechliche Wirkung statt nur das Flag.
      const enabled = entry.enabled !== false && cfg.charger_control !== false;
      return {
    ...
    // Wie beim Lader: Abschnitts-Schalter zuerst.
    const enabled = entry.enabled !== false && cfg[this._masterKeyFor(kind)] !== false;
  ...
  // Master switch per device class, set on the settings page ("EMS-Steuerung aktivieren"
  // under each section). It short-circuits before the per-device flag is ever looked at —
  // see chargerControl.js: `if (cfg.charger_control === false) return 0;` well above the
  // `enabled !== false` filter. Turning a single device on while its section is off used
  // to look like the toggle was broken.
  _masterKeyFor(kind) {
    return kind === 'charger' ? 'charger_control' : `${kind}_control`;
  },
  ...
  async setEmsDeviceEnabled(id, enabled) {
    ...
    // Switching a device on also lifts its section master if that was off — otherwise the
    // EMS returns before it ever reaches this device and the toggle does nothing. This
    // affects EVERY device of that class, so it is logged and reported back to the widget.
    let masterLifted = false;
    if (enabled) {
      const key = this._masterKeyFor(found.kind);
      if (cfg[key] === false) {
        cfg[key] = true;
        masterLifted = true;
        this.log(`[EMS] ${key} was off — switching it on so "${id}" can actually be controlled`);
      }
    }

    this.homey.settings.set('ems_config', cfg);
    this._stopTick(); this._startTick();
    return { ok: true, masterLifted };
  },
