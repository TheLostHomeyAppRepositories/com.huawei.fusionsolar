...
module.exports = {
  ...
  _trackChargeSession(charger, cfg, dtMs) {
    ...
    if (st.sessionActive && charger.powerW > 0) {
      const kwh = (charger.powerW / 1000) * (dtMs / 3600_000);
      st.sessionEnergyKwh += kwh;
      const price = this._getCurrentPrice(cfg);
      if (typeof price === 'number') {
        st.sessionCostSum   += kwh * price;
        st.sessionCostedKwh += kwh;
      }
    }
  ...
  _finalizeChargeSession(charger, st, cfg) {
    ...
    this._chargeSessions.push({
      ...
      energyKwh, cost, avgPrice, currency,
    });
