...
const {
  SIMPLE_MIN_RUN_MS, MIN_CHARGE_W, HIST, MODES, SIMPLE_STATE_KEY, SIMPLE_STATE_MAX_GAP_MS,
  SIMPLE_STATE_SAVE_MS, TRIGGER_BUDGET_MS, OVERFLOW_KEEP_W, OVERFLOW_START_W,
} = require('./constants');

// The mode each device type names when it is the only type the EMS is running. Keyed the
// way _simpleStateMaps keys its maps, so the two cannot drift apart.
const SIMPLE_MODE_BY_KIND = Object.freeze({
  heat_pump:    MODES.SOLAR_HP,
  boiler:       MODES.SOLAR_BOILER,
  pool:         MODES.SOLAR_POOL,
  dehumidifier: MODES.SOLAR_DEHUMIDIFIER,
  aircon:       MODES.SOLAR_AIRCON,
});
...
module.exports = {
  ...
  /**
   * Which simple devices the EMS is running right now, and what to call that.
   *
   * The mode is a statement about what the EMS is doing, so a device the user has taken
   * out of EMS control (`enabled: false`) has no place in it — whether or not that device
   * happens to be running. On top of that its state entry is frozen: _evaluateSimpleDevices
   * seeds the entry and only then skips a disabled device, so `isOn` keeps whatever it was
   * when the entry was created and nothing ever corrects it again.
   *
   * Both halves showed up in one field export. A pool taken out of the EMS, with no start
   * or stop card configured, still named the mode `solar_pool` — "Hydrojet Pool · Bat 55%"
   * at 21:00, no sun, battery discharging — and fired ems_mode_changed, the per-mode flow
   * and a history entry on the way there. The EMS announced a decision it had not made,
   * from a value it had stopped maintaining.
   *
   * Devices come in keyed as _simpleStateMaps keys the state, so a list cannot be paired
   * with the wrong map; that object's order is the order the status text lists. Nothing
   * running gives count 0 and mode null — every caller already gates on the count, and a
   * mode name for an empty set would be one more thing that could be believed.
   *
   * `text` is assembled here rather than at the call site because it is the same decision
   * said in words: it names the devices this method decided are running, and falls back to
   * a bare count only when a running device has no name. Built in _tickBody it could not
   * be driven by a test at all — and it is what reaches ems_status_text and the history.
   */
  _simpleActive(devicesByKind, socStr = '') {
    const maps  = this._simpleStateMaps();
    const names = [];
    const modes = [];
    let count = 0;
    for (const kind of Object.keys(maps)) {
      const map     = maps[kind];
      const running = (devicesByKind[kind] || [])
        .filter((d) => d.enabled !== false && map.get(d.id)?.isOn);
      if (!running.length) continue;
      count += running.length;
      modes.push(SIMPLE_MODE_BY_KIND[kind]);
      for (const d of running) names.push(d.name);
    }
    // One type running names itself. Several at once get the generic label — this used to
    // fall through to 'solar_hp' and read as "Solar heat pump" with no heat pump involved.
    const mode = count === 0 ? null : (modes.length === 1 ? modes[0] : MODES.SOLAR_MULTI);
    const named = names.filter(Boolean);
    let text = '';                                   // nothing running names nothing
    if (named.length)   text = `${named.join(', ')}${socStr}`;
    else if (count > 0) text = `${count} Gerät${count > 1 ? 'e' : ''} aktiv${socStr}`;
    return { count, mode, names, text };
  },
  ...
  _restoreSimpleStates() {
    ...
    // Which devices the settings still know about, per kind. A device removed from the
    // configuration otherwise keeps its state entry for good: nothing prunes these maps, so
    // it is restored on every boot and listed in every diagnostics export. The field export
    // of 2026-08-30 still carried a pool paired and removed long before — no name, no
    // measurements, and no way for the owner to get rid of it.
    //
    // Skipped entirely when the configuration carries no device lists at all. That is what a
    // failed or empty settings read looks like, and pruning against it would throw away the
    // timers of every device that IS configured — the opposite of what this function exists
    // for. Restoring one stale entry too many is the cheaper mistake.
    // The catch is the whole policy: whatever goes wrong while working out what is
    // configured — a failed settings read, no _getConfig at all — leaves `configured` null
    // and prunes nothing. A second guard for the missing method would say the same thing
    // twice and could not be tested apart from this one.
    let configured = null;
    try {
      const cfg = this._getConfig() || {};
      const lists = Object.keys(this._simpleStateMaps())
        .map((kind) => [kind, Array.isArray(cfg[`${kind}_devices`]) ? cfg[`${kind}_devices`] : null]);
      if (lists.some(([, l]) => l && l.length)) {
        configured = {};
        for (const [kind, l] of lists) configured[kind] = new Set((l || []).map((d) => d && d.id));
      }
    } catch (_) { configured = null; }
    ...
    for (const [key, map] of Object.entries(this._simpleStateMaps())) {
      ...
      for (const [id, st] of Object.entries(stored)) {
        ...
        if (configured && !configured[key].has(id)) { dropped++; continue; }
        const clean = {};
    ...
    // Said out loud: a silently vanishing entry is indistinguishable from a bug, and this
    // line is the only place the owner learns that the leftover is finally gone.
    if (dropped) this.log(`[EMS] simple-device state: dropped ${dropped} entry/entries for devices no longer configured`);
  },
  ...
  async _simpleDeviceSetOn(id, name, on, stateMap, startCard, stopCard, tokenName) {
    ...
    // Has this run ever actually been seen running? The start card is a flow the USER
    // owns, and a flow is free to decline — this owner's air-conditioning flow only acts
    // when the room is warm enough. Deliberately not persisted (SIMPLE_STATE_FIELDS): a
    // restart in the middle of a run cannot know what happened while it was down, and
    // `undefined` falls back to the older, cautious reading below.
    if (on) { st.startedAt = Date.now(); st.seenOn = false; }
    else { st.startedAt = null; st.seenOn = undefined; st.lastEmsStopAt = Date.now(); }
    st.isOn = on;
  ...
  /**
   * @param {number|null} allowanceW  What is LEFT of the surplus ramp's share after the
   *   devices ahead of these in the priority order took theirs. null when no ramp is
   *   configured, and then nothing below changes.
   */
  async _evaluateSimpleDevices(battery, gridW, devices, stateMap, startCard, stopCard, tokenName, cfg, allowanceW = null) {
    if (!devices.length) return 0;

    const { batHardStop, minSoc: hardStopSoc } = this._batteryZones(cfg, battery);
    ...
    for (const device of devices) {
      ...
      if (this._warmupDone && device.actualOn !== null) {
        ...
        // Seen running at least once during this EMS run — the one fact that separates the
        // two ways a run can end (see below). Recorded before the branches so it also
        // counts sightings made inside the startup grace.
        if (device.actualOn && st.isOn) st.seenOn = true;
        if (!device.actualOn && st.isOn && !startGraceActive) {
          // Both endings used to read "switched off outside the EMS", and that cost a field
          // report: an air conditioner the EMS started four times, twice with no effect,
          // was reported as a broken device when the real cause was a condition in the
          // owner's own start flow. If the device was never once seen on, nobody switched
          // anything off — the start simply never took hold, and the EMS finds out the
          // moment its startup grace expires. `undefined` means the run predates this
          // record (restored state, first tick), and keeps the older reading.
          const neverTookHold = st.seenOn === false;
          this.log(`[EMS] ${tokenName} ${device.id}: ${neverTookHold ? 'start had no effect' : 'external OFF detected'} — adopting state`);
          this._addHistoryEvent(HIST.DEVICE, neverTookHold ? 'start_no_effect' : 'manual_off', device.name, device.id);
      ...
      // The surplus ramp as a ceiling. Everything above creates the figure; this bounds it.
      // A running device's own draw is added back on the line above — correctly, since
      // stopping it would turn that draw into export — but that also means a device already
      // running justifies itself, exactly as the charger did in the field on 2026-08-29.
      // The share has to be able to say no to a device that is already on, or it only ever
      // gates starts.
      //
      // Math.min, never max: like every guard here it can tighten and never loosen, and a
      // null allowance (no ramp configured) leaves the old figure untouched.
      const rawEffectiveW = runningExportW + hpPowerW;
      const effectiveW = (allowanceW === null || allowanceW === undefined)
        ? rawEffectiveW
        : Math.min(rawEffectiveW, Math.max(0, allowanceW));
      ...
      //
      // The order mirrors the wantOn hierarchy above, and has to: this chain reports which
      // constraint is holding the device, so a branch placed above the one that actually
      // decides names a constraint that is not the binding one.
      //
      // The hard stop had no branch at all, which is how a log came to read
      // "start-sustain pending (120s)" for three devices at 00:36 on a battery at 2 % —
      // measured 2026-08-22. Nothing was pending: five lines up, `batHardStop && !batOverflow`
      // had already set wantOn = false, and no surplus figure can lift that. It read worse
      // than merely wrong, because the line had said "solar-forecast gate — holding start"
      // until midnight; the gate then opened for the new day, the phase changed, and the log
      // announced movement towards a start while the real blocker had not shifted an inch.
      let diagKey = null, diagMsg = null;
      if (batHardStop && !batOverflow) {
        diagKey = 'hardstop';
        diagMsg = `battery hard stop — SoC ${battery.soc}% below ${hardStopSoc}%, no device runs`;
      } else if (inMaxRun) {
