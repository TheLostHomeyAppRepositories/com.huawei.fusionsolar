...
module.exports = {
  ...
  // Called at the end of every tick. Writes only when something actually changed, so a
  // steady state costs one JSON.stringify per tick and no settings write at all.
  _saveSimpleStates() {
    const maps = {};
    for (const [key, map] of Object.entries(this._simpleStateMaps())) {
    ...
    if (body === this._lastSimpleStateJson) return;
    this._lastSimpleStateJson = body;
    try {
      this.homey.settings.set(SIMPLE_STATE_KEY, { savedAt: Date.now(), maps });
    } catch (e) {
  ...
  async _evaluateSimpleDevices(battery, gridW, devices, stateMap, startCard, stopCard, tokenName, cfg) {
    ...
    const overflowThreshold = anyRunning ? MIN_CHARGE_W / 2 : MIN_CHARGE_W * 2;
