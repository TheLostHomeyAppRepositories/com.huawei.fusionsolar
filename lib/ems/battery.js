...
module.exports = {
  ...
  /**
   * The usable capacity of one configured battery, in kWh.
   *
   * Four places need this number — the adaptive forecast gate, its diagnostic twin,
   * price-optimised charging and the widget — and each of them used to write
   * `Number(d.capacity_kwh) || 0` for itself. One accessor instead, so choosing a
   * capability cannot reach three of them and miss the fourth.
   *
   * Lives here rather than on the device, because all three callers are mixins in this
   * folder that are composed and tested on their own; on the device it was simply absent
   * whenever they were.
   *
   * The typed number stays the fallback, and stays the whole answer for a battery whose
   * capability was never chosen: an EMMA battery, a LUNA that does not answer register
   * 37758, or a configuration saved before any of this existed.
   */
  _capacityKwhFor(bd) {
    if (!bd) return 0;
    const learned = this._capacityByBattery && this._capacityByBattery[bd.id];
    if (bd.cap_capacity && typeof learned === 'number' && Number.isFinite(learned) && learned > 0) {
      return learned;
    }
    return Number(bd.capacity_kwh) || 0;
  },
