...
module.exports = {
  ...
  // How long the surplus path leaves a charger alone after it ignored a start twice.
  // Andi's call, and the trade is his to make: the back-off costs nothing while a charger
  // is charging (any draw clears it) but it does delay the next offer of surplus to one
  // that has started behaving — a car that wakes up, a flow that gets fixed. Short is
  // forgiving, long is quiet. At FLIP_COOLDOWN_MS the two run in parallel and this adds
  // little beyond making the intent explicit in the log.
