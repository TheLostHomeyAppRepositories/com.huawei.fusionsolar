...
module.exports = {
  ...
  _batteryPriceMode(bd, socNow, cfg, nowMs = Date.now()) {
    ...
    if (isChargeNow) return { mode: 'charge', reason: 'in a selected cheap slot within the lookahead window', chargeSlots, reserveSlots: [], ...solarInfo };
    ...
    if (reserveHours > 0) {
      ...
      if (candidates.length) {
        ...
        reserveSlots = candidates.filter((s) => selected.has(s.start)).sort((a, b) => a.start - b.start);
        if (!this._priceSlotSelectedNow(selected, candidates, nowMs)) {
          return { mode: 'hold', reason: `reserving capacity for the most expensive upcoming hours${solarSuffix}`, chargeSlots, reserveSlots, ...solarInfo };
        }
        return { mode: 'normal', reason: `in a top-expensive slot — discharge allowed${solarSuffix}`, chargeSlots, reserveSlots, ...solarInfo };
      }
    }

    return { mode: 'normal', reason: solarBlocks ? solarSuffix.replace(/^ — /, '') : 'nothing to do', chargeSlots, reserveSlots, ...solarInfo };
  },
