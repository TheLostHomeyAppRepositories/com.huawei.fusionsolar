...
module.exports = {
  ...
  _batteryPriceMode(bd, socNow, cfg, nowMs = Date.now()) {
    ...
    if (isChargeNow) return { mode: 'charge', reason: 'in a selected cheap slot within the lookahead window', chargeSlots, reserveSlots: [] };
    ...
    if (reserveHours > 0) {
      ...
      if (candidates.length) {
        ...
        if (!this._priceSlotSelectedNow(selected, candidates, nowMs)) {
          return { mode: 'hold', reason: 'reserving capacity for the most expensive upcoming hours', chargeSlots, reserveSlots };
        }
        return { mode: 'normal', reason: 'in a top-expensive slot — discharge allowed', chargeSlots, reserveSlots };
      }
    }

    return { mode: 'normal', reason: 'nothing to do', chargeSlots, reserveSlots };
  },
