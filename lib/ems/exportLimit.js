...
const {
  EXPORT_LIMIT_MIN_EXPORT_W, EXPORT_LIMIT_HOLD_MS, MODES, HIST, TRIGGER_BUDGET_MS,
} = require('./constants');
...
module.exports = {
  ...
  async _fireExportLimitTrigger(cfg, cardId, logLabel) {
    ...
    for (const invId of invIds) {
      ...
      // Bounded per inverter: this loop is on the tick path, and a multi-inverter install
      // would otherwise wait out one slow flow after another before the loop moves on.
      await this._settleWithin(
        ...
        TRIGGER_BUDGET_MS, `export limit ${logLabel} (inverter ${invId})`);
    }
  },
  ...
  async _evaluateExportLimit(cfg, battery, gridW) {
    ...
    // The price reason must NOT require export to keep holding. Once the limit bites, the
    // inverter curtails and export stops — so tying the hold to `exporting` released the
    // limit after the hold timer, export resumed at a still-negative price, and the rule
    // re-armed: a five-minute duty cycle that fed in for part of every period, which is
    // precisely what it exists to prevent. The SOC reason does need it: there, "no export"
    // is real evidence that the surplus is gone.
    const wantHold = priceLow || (exporting && socHold);
