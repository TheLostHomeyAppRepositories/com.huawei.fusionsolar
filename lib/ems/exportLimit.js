...
module.exports = {
  ...
  async _evaluateExportLimit(cfg, battery, gridW) {
    ...
    if (battery.soc === null || gridW === null) return;
    ...
    const wantOn   = (battery.soc >= trigSoc && exporting) || (priceLow && exporting);
    const wantHold = (battery.soc >= deactSoc && exporting) || (priceLow && exporting);
    ...
    if (!this._exportLimitActive && wantOn) {
      // SOC wins the label when both reasons apply: it is the reason the user configured first
      // and the one whose thresholds explain the following OFF.
      const bySoc  = battery.soc >= trigSoc;
      const reason = bySoc
      ...
      this._exportLimitActivatedAt = null;
      await this._persistExportLimit();
      this.log(`[EMS] export limit OFF — SOC ${Math.round(battery.soc)}% < ${deactSoc}%, price high or no export`);
