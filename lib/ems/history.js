...
module.exports = {
  ...
  _noteTickSkip(tickMs, now = Date.now()) {
    ...
    // The running tick's own elapsed time and phase are the only figures that explain the
    // overrun, and the only ones available while it is still running. avg and max are
    // written when a tick FINISHES, so they describe every tick except the guilty one —
    // a five-minute tick was once announced as "max 1755 ms", which sent us looking in
    // the wrong place entirely.
    const runningMs = this._tickStartedAt ? now - this._tickStartedAt : null;
    const running = runningMs !== null ? `${Math.round(runningMs / 1000)}s` : 'unknown';
    this.log(`[EMS] tick overrun — 2 skipped in a row `
      + `(running ${running} in phase "${this._tickPhase || 'unknown'}", `
      + `interval ${tickMs} ms, avg ${this._diag.avgTickMs} ms over completed ticks)`);
