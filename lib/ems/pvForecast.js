...
module.exports = {
  ...
  _forecastGateBlocksStarts(cfg, battery, nowMs = Date.now()) {
    ...
    if (!(cfg.battery_devices || []).length) return false;
    if (this._pvForecastStale(nowMs)) return false;

    const remaining = this._pvForecastRemainingTodayKwh(nowMs);
    let threshold;
    if (mode === 'adaptive') {
      const cap = Number(cfg.battery_capacity_kwh) || 0;
      if (cap <= 0 || battery.soc === null || battery.soc === undefined) return false;
