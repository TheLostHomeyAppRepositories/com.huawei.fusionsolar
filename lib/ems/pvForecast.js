...
module.exports = {
  ...
  async _maybeFetchPvForecast(cfg) {
    ...
    const resourceId = (cfg.solcast_resource_id || '').trim();
    if (!apiKey || !resourceId) return;

    const now = Date.now();
    if (now < this._pvForecastBackoffUntil) return;
    if (this._pvForecastFetchedAt && (now - this._pvForecastFetchedAt) < PV_FORECAST_MIN_INTERVAL_MS) return;

    try {
      const json  = await this._solcastGet(resourceId, apiKey);
      const slots = this._parseSolcastForecasts(json);
      if (!slots.length) throw new Error('empty / unparseable forecast response');
      this._pvForecast          = slots;
      this._pvForecastFetchedAt = now;
      this._pvForecastError     = null;
      this._pvForecastLoggedErr = false;
      await this.setStoreValue('pvForecast', { slots, fetchedAt: now }).catch(() => {});
      this.log(`[EMS] PV forecast updated: ${slots.length} slots, remaining today ≈ ${this._pvForecastRemainingTodayKwh().toFixed(1)} kWh`);
  ...
  _pvForecastSummary(nowMs = Date.now()) {
    ...
    const configured = !!(cfg.solcast_api_key && cfg.solcast_resource_id);
