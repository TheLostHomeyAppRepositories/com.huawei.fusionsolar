...
module.exports = {
  ...
  _forecastGateState(cfg, battery, nowMs = Date.now()) {
    const off = { blocked: false, mode: 'off', remainingKwh: null, thresholdKwh: null };
    const mode = cfg.forecast_gate_mode || 'off';
    if (mode === 'off') return off;
    if (!(cfg.battery_devices || []).length) return off;
    if (this._pvForecastStale(nowMs)) return off;
    ...
    if (mode === 'adaptive') {
      ...
      if (cap <= 0 || battery.soc === null || battery.soc === undefined) return off;
  ...
  _announceForecastGate(cfg, battery, nowMs = Date.now()) {
    ...
    if (blocked === this._forecastGateOn) return false;
