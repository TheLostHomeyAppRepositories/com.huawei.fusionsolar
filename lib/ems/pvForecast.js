...
module.exports = {
  ...
  async _updatePvForecastCapabilities(nowMs = Date.now()) {
    ...
    const today = Math.round(this._pvForecastRemainingTodayKwh(nowMs) * 10) / 10;
    const tomo  = Math.round(this._pvForecastTomorrowKwh(nowMs) * 10) / 10;
    const now   = Math.round(this._pvForecastNowKw(nowMs) * 100) / 100;
