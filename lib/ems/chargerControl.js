    ...
    if (cur > 0 && hasMeter && drawnW < CHARGER_LIVE_W) {
      ...
      if (silentMs >= CHARGER_START_GIVEUP_MS) {
        this.log(`[EMS] charger ${charger.id}: drew nothing for ${Math.round(silentMs / 1000)}s `
          + `after two start commands — releasing its ${held.allocatedW}W of surplus and leaving `
          + `it alone for ${CHARGER_IGNORED_BACKOFF_MS / 60_000} min. Check that the flow on `
          + `ems_start_charger really starts the charger, not only sets its current.`);
        await this._chargerStop(charger.id);
        st.startBlockedUntil = now + CHARGER_IGNORED_BACKOFF_MS;
