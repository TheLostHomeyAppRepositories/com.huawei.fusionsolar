...
module.exports = {
  ...
  _gridImportClaimAmps(cfg, minAmps, desiredAmps, phases) {
  ...
  async _evaluateEvChargers(battery, gridW, chargers, cfg, pvW = null, houseW = null) {
    ...
    if (this.getCapabilityValue('charge_now') === true) {
      ...
      for (const c of chargers) {
        ...
        const grantedAmps = this._gridImportClaimAmps(cfg, c.minAmps, c.maxAmps, c.phases);
        const st = this._getChargerState(c.id);
        if (grantedAmps <= 0) {
          if (st.currentAmps !== null) await this._chargerStop(c.id);
          continue;
        }
        if (this._warmupDone && (st.currentAmps !== grantedAmps || st.currentPhases !== c.phases)) {
          await this._chargerSetAmps(c.id, grantedAmps, c.phases);
        }
        instantGranted.push({ c, amps: grantedAmps });
      }
      const parts = instantGranted.map(({ c, amps }) => `${amps}A/${c.phases}ph`).join(' + ');
      await this._setMode(MODES.INSTANT_EV, parts);
      return instantGranted.reduce((s, { c, amps }) => s + amps * c.phases * 230, 0);
    }
    ...
    let alwaysW = 0;
    const alwaysGrantedAmps = new Map(); // c.id → granted amps, for the status text below
    ...
    if (alwaysChargers.length) {
      for (const c of alwaysChargers) {
        ...
        alwaysGrantedAmps.set(c.id, grantedAmps);
        alwaysW += grantedAmps * c.phases * 230;
      }
      ...
      if (!chargers.length) {
        ...
        const aParts = connAlways.map((c) => `${alwaysGrantedAmps.get(c.id)}A/${c.phases}ph`).join(' + ');
    ...
    if (!batOverflowMode && !batReserveMode && offpeakChargers.length && offpeakWin.active) {
      ...
      if (!solarCanClaim) {
        ...
        for (const c of offpeakChargers) {
          ...
          const st         = this._getChargerState(c.id);
          const grantedAmps = this._gridImportClaimAmps(cfg, c.minAmps, opAmps, opPhases);
          if (grantedAmps <= 0) {
            if (st.currentAmps !== null) await this._chargerStop(c.id);
            continue;
          }
    ...
    if (!batOverflowMode && !batReserveMode && priceChargers.length) {
      ...
      if (!solarCanClaim) {
        for (const c of priceChargers) {
          ...
          // Shared grid-import budget (cfg.price_charge_max_grid_kw, default unlimited):
          // skip if adding this charger would exceed what's left after the battery's
          // own committed price-charging (evaluated earlier this tick) and any other
          // price/low-tariff charger already fitted below. It simply doesn't grid-charge
          // this tick — falls through to the normal solar-surplus loop like any charger
          // that decided not to charge.
          if (!decision.shouldCharge) continue;
          const fits = this._priceChargeClaimBudget(cfg, chargerPowerW);
          this._debugLog(`charger ${c.id}: grid-charge budget ${fits ? 'granted' : 'DENIED — no budget left'} (${chargerPowerW}W)`);
          if (!fits) continue;
          // Whole-house grid-import ceiling — layered on top of the price budget above;
          // gracefully reduces amps (rather than skipping outright) if the full-power
          // draw would exceed the house's main-fuse safety limit.
          const grantedAmps = this._gridImportClaimAmps(cfg, c.minAmps, c.maxAmps, phases);
      ...
      if (dual.configured && !dual.isHigh) {
        ...
        if (!solarCanClaim) {
          ...
          for (const c of lowTariffChargers) {
            ...
            this._debugLog(`charger ${c.id}: grid-charge budget ${fits ? 'granted' : 'DENIED — no budget left'} (${chargerPowerW}W)`);
            if (!fits) continue;
            // Whole-house grid-import ceiling, layered on top — see P3b for rationale.
            const grantedAmps = this._gridImportClaimAmps(cfg, c.minAmps, c.maxAmps, phases);
            if (grantedAmps <= 0) continue;
