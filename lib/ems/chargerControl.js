...
const {
  ...
  CHARGER_LIVE_W, CHARGER_STOP_WARN_TICKS, TRIGGER_BUDGET_MS,
} = require('./constants');
...
module.exports = {
  ...
  async _runPriorityLoop(battery, gridW, chargers, cfg, pvW, houseW, priorityOrder, simpleEval) {
  ...
  async _evaluateEvChargers(battery, gridW, chargers, cfg, pvW = null, houseW = null) {
    ...
    for (const c of chargers) {
      ...
      if (car && car.soc !== null && car.target !== null) {
        ...
        // A car is assigned but one of the two numbers is missing, so the check above
        // cannot run and the charger falls through to normal allocation — it charges.
        // That is the right call (an unreadable car may well need the energy), but it has
        // to be visible: a 100% car that only reports its SoC once charging wakes it looks
        // exactly like this for the two minutes before the truth arrives, and the log used
        // to show only the start and, later, the stop, with nothing to connect them.
        // Once per blind spell, not once per tick.
    ...
    if (!batOverflowMode && pvW !== null && houseW !== null) {
      ...
      // Which of the two estimates wins is a STATE, not an event. Logged on every tick it
      // repeated the same sentence for hours on end — in one field log it is the single
      // most frequent line between 18:00 and 20:40, once every 20 s. The changeover is the
      // part worth reading; the watts are in the diagnostics panel either way.
      if (pvWins !== this._pvBudgetWon) {
        this._pvBudgetWon = pvWins;
        this.log(pvWins
  ...
  async _stepCharger(charger, budgetW, configPhases, now, gridW, forcedDown) {
    ...
    const drawnW = Number.isFinite(charger.powerW) ? charger.powerW : 0;
