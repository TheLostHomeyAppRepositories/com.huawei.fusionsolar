...
/**
 * The charge target typed into a car's settings, or null when the field is empty.
 *
 * Empty means no target, not zero and not a default: a car whose owner has not named a
 * limit is charged without one. The EMS used to write an 80 here on the owner's behalf and
 * then enforce it, which is the same fault as presenting a missing reading as a
 * measurement — only worse, because it changed what the hardware did.
 */
function carTargetFromConfig(car) {
...
module.exports = {
  ...
  //   measure_car_target_soc.<carId>  — target SOC. Three sources in order: the vehicle's
  //     own target_soc_capability, a value set through the ems_set_car_target_soc flow, or
  //     the number typed into the car's settings. None of them set means NO target, and a
  //     car with no target is not held back — see carTargetFromConfig below.
  async _updateCarCapabilities(cfg) {
    ...
    for (const car of cars) {
      ...
      if (pinned) {
        ...
        // Last: the number typed into the car's settings. It used to be an 80 this code
        // invented for any car without a target capability, which silently stopped every
        // such car at 80% — a limit nobody had chosen, enforced as if they had. Reported
        // on an Audi Q4 that sat at exactly that figure.
        if (tgt === null && carTargetFromConfig(car) !== null) tgt = clamp(carTargetFromConfig(car));
      }
      ...
      states.push({
        ...
        // Whether a target was ever asked for. A null target with a source configured is a
        // reading that has not arrived yet; a null target with no source is a car the user
        // chose not to limit. The charger says something different about each.
        targetConfigured: !!car.target_soc_capability
          || typeof this._carTargets[car.id] === 'number'
          || carTargetFromConfig(car) !== null,
